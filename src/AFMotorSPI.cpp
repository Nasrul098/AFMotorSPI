...
#if defined(AF_SPI_COMPATIBLE)
    // use PWM from timer1A on PB1 (Arduino pin #9)
//  Clear all modified bits
    TCCR1A &= ~(_BV(COM1A1) | _BV(COM1A0) | _BV(WGM11) | _BV(WGM10));
//  Set necessary bits (turn on OC1A, fast or phase-correct PWM)
    TCCR1A |= _BV(COM1A1) | _BV(WGM10);
//  Set prescaler and fast or phase-correct PWM
    TCCR1B = (freq & (PHASE_CORRECT-1)) | (freq & PHASE_CORRECT ? 0 : _BV(WGM12));
    OCR1A = 0;
#else       //defined(AF_SPI_COMPATIBLE)
    // use PWM from timer2A on PB3 (Arduino pin #11)
//  Clear all modified bits
    TCCR2A &= ~(_BV(COM2A1) | _BV(COM2A0) | _BV(WGM21) | _BV(WGM20));
//  Set necessary bits (turn on OC2A, fast or phase-correct PWM)
    TCCR2A |= _BV(COM2A1) | _BV(WGM20) | (freq & PHASE_CORRECT ? 0 : _BV(WGM21));
//  Set prescaler
    TCCR2B = freq & (PHASE_CORRECT-1);
    OCR2A = 0;
#endif       //defined(AF_SPI_COMPATIBLE)
...
//  Set necessary bits (turn on OC2B, fast or phase-correct PWM)
    TCCR2A |= _BV(COM2B1) | _BV(WGM20) | (freq & PHASE_CORRECT ? 0 : _BV(WGM21));
//  Set prescaler
    TCCR2B = freq & (PHASE_CORRECT-1);
    OCR2B = 0;
#elif defined(__AVR_ATmega1280__)
...
inline void initPWM3(uint8_t freq) {
    ...
    //We cannot change mode and prescaler of timer0, because it's used by millis() and delay()
    TCCR0A &= ~_BV(COM0A0); TCCR0A |= _BV(COM0A1);	//only connect OC0A
...
inline void initPWM4(uint8_t freq) {
    ...
    //We cannot change mode and prescaler of timer0, because it's used by millis() and delay()
    TCCR0A &= ~_BV(COM0B0); TCCR0A |= _BV(COM0B1);	//only connect OC0B
